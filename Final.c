...
struct usuario cadastro_cliente[max];			//	Defini��o das estruturas de dados.
...
void busca_prod(){	/*	Possivelmente, � interessante que deixemos essa fun��o mais completa... Com op��es de editar qualquer dado do produto	*/
...
/*	Vamos ter que alterar essa fun��o para uma fun��o que cadastra um �nico cliente, e criar outra fun��o que chama essa v�rias vezes	*/
...
void saida_de_produto() {
	...
	if (stricmp(controller, "nao") != 0) {
		...
		if (posicao_do_produto != NULL) {	//	Verifica��o de sucesso ao encontrar o produto com determinado nome
...
int busca_de_produtos (char *p) {	/*	*p � o parametro que receber� a string enviada como argumento � fun��o	*/

	int posicao, i, controller = 0;

	for (i = 0; i < max; i++) {		//	Inicia-se a verifica��o pelo vetor de produtos, procurando algum produto do nome recebido por parametro pela funcao
